...
function notifyCakeFinished() {
  try {
    sessionStorage.setItem('zulaikhaFromCake', '1');
  } catch (_) {
    /* ignore */
  }
  if (window.parent && window.parent !== window) {
    window.parent.postMessage({ type: 'zulaikha-cake-finished' }, '*');
  } else {
    window.location.href = 'index.html?fromcake=1';
  }
}
...
const MASTER_TL = timeline({
  ...
  onComplete: () => {
    gsap.delayedCall(0.7, notifyCakeFinished);
